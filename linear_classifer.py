...
def cross_entropy_loss(probs, target_index):
    ...
    if len(probs.shape) == 1:
        ...
        #batch
        return -np.sum(np.log(probs[:, target_index])) / probs.shape[0]
...
def linear_softmax(X, W, target_index, tol = 1e-10):
    '''
    ...
    loss = -np.sum(target_index*np.log(probs+tol))
    dW = probs - target_index
